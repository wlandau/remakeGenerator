...
Above, `test_mode()` sets the `downsize` option to `TRUE`, which is a signal to the `downsize()` function. The command `downsize(A, ...)` says "downsize `A` to a some other object when `getOption("downsize")` is `TRUE`". To switch to the full scaled-up workflow, replace `test_mode()` with `production_mode()`. Unfortunately, [`remake`](https://github.com/richfitz/remake) does not rebuild targets in response to changes to global options, so you should manually run `remake::make("clean")` to start from scratch whenever you scale up or down.
